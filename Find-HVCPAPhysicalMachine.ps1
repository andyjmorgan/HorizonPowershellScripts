...
.SYNOPSIS
This is a simple Powershell script to search a Horizon Cloud Pod Environment for a machine name, or a partial machine name.

.DESCRIPTION
The script will only return the machine Name, pool name, assigned user(s) and pod name.

.EXAMPLE
Find-HVCPAPhysicalMachine -connectionServerName connectionserver.lab.local -machineName machineName -credential (get-credential)

.NOTES
Example output:

PoolName MachineName     AssignedUsers  PodName
-------- -----------     -------------  -------
manual   m10d1.lab.local lab.local\andy Pod 2  

.LINK
https://github.com/andyjmorgan/HorizonPowershellScripts
...
$hvserver = Connect-HVServer $connectionServerName -Credential $Credential
